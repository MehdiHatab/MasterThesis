...
f,mu_re,mu_im = np.loadtxt('mu_ferrite75.txt',unpack=True)

xml_file = 'choke_DM_freq.xml'
...
for ll in range(len(f)):
    ...
    for item in content: #rewrites file content from list 
        infile.write("%s" % item)
    infile.close()
    call('/home/hatab/Devel_MA/CFS_BIN/build_opt/bin/cfs -t6 -p {0} choke_DM_75_f_{1}'.format(xml_file,ll),shell=True)
