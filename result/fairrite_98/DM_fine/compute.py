...
f,mu_re,mu_im = np.loadtxt('fairrite_98.txt',unpack=True)
...
for ll in range(len(f)):
    if ll >= 0:
        ...
        for item in content: #rewrites file content from list 
            infile.write("%s" % item)
        infile.close()
        call('/home/hatab/Devel_MA/CFS_BIN/build_opt/bin/cfs -t6 -p {0} choke_DM_98_f_{1}'.format(xml_file,ll),shell=True)
